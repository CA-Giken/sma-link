...
let Indicator={"/sensors/stat":0,"/rsocket/enable":0,"/diskfree/stat":0};
...
setImmediate(async function(){
  ...
  rosNode.subscribe('/dashboard/indicator',std_msgs.String,(msg)=>{
    let jss=msg.data.replace(/\)/g, ']').replace(/\(/g, '[').replace(/\'/g, '\"');
    try{
      let sc=JSON.parse(jss);
      Object.assign(Indicator,sc);
    }
    catch(e){
      ros.log.error('/report parse error:'+jss);
    }    
 });
  ...
  client.on('data',async function(data){
    ...
    if (Indicator.hasOwnProperty('/sensors/stat')){
      try{
        client.write("status,"+Indicator['/sensors/stat']+","+Indicator['/rsocket/enable']+","+Indicator['/diskfree/stat']+protocol.lf,'utf8',()=>{
        });
        }
      catch(e){
        ros.log.error('r_client::send statud error '+e);
        return;        
      }
      }
